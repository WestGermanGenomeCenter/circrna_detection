"""
linear_report.py — Linear RNA-seq QC Interactive HTML Report
=============================================================
Produces a self-contained HTML report from a featureCounts output file.
Used as a Snakemake script (snakemake.input / snakemake.output) OR as a
standalone CLI tool:

    python linear_report.py <featureCounts_file> <output.html>
...
Input format: featureCounts output (tab-separated, optional leading '#' comment lines,
first 6 columns are metadata, remaining columns are sample BAM paths → counts).
"""
...
def load_counts(path):
    """
    Load a featureCounts output file.
    Columns 0-5 are metadata (Geneid, Chr, Start, End, Strand, Length).
    Remaining columns are BAM file paths → one per sample.

    Sample name extraction strategy:
      - STAR always writes a file called Aligned.sortedByCoord.out.bam inside
        a per-sample directory, so the sample name lives in the *parent directory*
        of the BAM, not in the filename itself.
      - For other aligners the sample name is encoded in the filename; known
        BAM suffixes are stripped from the right.
    """
    df = pd.read_csv(path, sep="\t", comment="#")
    raw   = df.iloc[:, 6:].to_numpy(dtype=float)
    genes = df["Geneid"].astype(str).values

    # Filenames produced by STAR — sample ID is in the parent directory
    STAR_FILENAMES = {"Aligned.sortedByCoord.out.bam", "Aligned.out.bam"}

    # Suffixes for non-STAR aligners where the sample name is in the filename
    BAM_SUFFIXES = (
    ...
    for c in df.columns[6:]:
        ...
        if bn in STAR_FILENAMES:
            # STAR output: use the per-sample subdirectory name
            name = os.path.basename(os.path.dirname(c))
        else:
            # Generic BAM: strip known suffix from the filename
            name = bn
...
<title>Linear RNA-seq QC Report</title>
...
<body>
<aside class="sidebar">
  <h1>Linear RNA-seq<br>QC Report</h1>
  __NAV__
...
def main():
    ...
    try:
        ...
        counts_f = str(snakemake.input.counts)   # noqa: F821
        out_html = str(snakemake.output.html)     # noqa: F821
    except NameError:
        # Running from the command line
        if len(sys.argv) < 3:
            print("Usage: python linear_report.py <featureCounts_file> <output.html>")
            sys.exit(1)
